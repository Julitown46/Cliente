window.onload = ordenAlfabetico(true);

function ordenAlfabetico(orden2) {
    let acordion = document.getElementById("accordionComunidades");
    acordion.innerHTML = '';

    let comunidadesOrdenadas = comunidades.sort((a, b) => a.label.localeCompare(b.label));
    ...
    comunidadesOrdenadas.forEach(comunidad => {
        ...
        let provinciasOrdenadas = comunidad.provinces.sort((a, b) => a.label.localeCompare(b.label));
...
function ordenPorNumeroDeCiudades(orden2) {
    let acordion = document.getElementById("accordionComunidades");
    acordion.innerHTML = '';

    let comunidadesOrdenadas = comunidades.sort((a, b) => {
    ...
    comunidadesOrdenadas.forEach(comunidad => {
        acordion.innerHTML += `<div class="accordion-item">
                <h2 class="accordion-header" id="header${comunidad.label}">
                    <button class="accordion-button" type="button" data-bs-toggle="collapse"
                        ...
                        ${comunidad.label} (${comunidad.provinces.reduce((sum, provincia) => sum + provincia.towns.length, 0)} cities total)
                    </button>
                </h2>
                <div id="collapse${comunidad.label}" class="accordion-collapse collapse show" aria-labelledby="header${comunidad.label}"
                    data-bs-parent="#accordionComunidades">
                    <div class="accordion-body">
                        <div class="accordion sub-accordion" id="accordion${comunidad.label}">
        ...
        let provinciasOrdenadas = comunidad.provinces.sort((a, b) => b.towns.length - a.towns.length);
        ...
        provinciasOrdenadas.forEach(provincia => {
            subAcordion.innerHTML += `<div class="accordion-item">
                    ...
                    <div id="collapse${provincia.label}" class="accordion-collapse collapse" aria-labelledby="header${provincia.label}"
                        data-bs-parent="#accordion${comunidad.label}">
                        <div class="accordion-body" id="ciudades${provincia.label}">
